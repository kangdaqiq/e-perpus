...
<div class="mb-8">
    ...
    <h2 class="text-2xl md:text-3xl font-bold text-slate-800 dark:text-slate-100">Tambah Admin Baru</h2>
    <p class="text-xs md:text-sm text-slate-400 font-medium">Buat kredensial admin baru untuk salah satu sekolah.</p>
</div>
    ...
    <form action="{{ route('superadmin.admins.store') }}" method="POST" class="space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <input type="text" name="username" id="username" value="{{ old('username') }}" required placeholder="budi_perpus" class="w-full px-4 py-3 bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-800 rounded-2xl text-sm font-semibold focus:outline-none focus:border-indigo-500 transition-all duration-200 @error('username') border-rose-500 @enderror">
                @error('username')
                    <p class="text-rose-500 text-xs font-semibold mt-1">{{ $message }}</p>
                @enderror
            </div>
        ...
        <div>
            ...
            <input type="password" name="password" id="password" required placeholder="Minimal 6 karakter" class="w-full px-4 py-3 bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-800 rounded-2xl text-sm font-semibold focus:outline-none focus:border-indigo-500 transition-all duration-200 @error('password') border-rose-500 @enderror">
            @error('password')
                <p class="text-rose-500 text-xs font-semibold mt-1">{{ $message }}</p>
            @enderror
        </div>
