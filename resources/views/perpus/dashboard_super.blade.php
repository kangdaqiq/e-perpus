    ...
    <div class="overflow-x-auto">
        <table class="w-full text-left border-collapse">
            ...
            <tbody class="divide-y divide-slate-100 dark:divide-slate-800 text-sm font-medium text-slate-700 dark:text-slate-300">
                    ...
                    <tr class="hover:bg-slate-50/50 dark:hover:bg-slate-800/20 transition-colors">
                        ...
                        <td class="p-6 text-center">
                            <div class="flex items-center justify-center gap-2">
                                <form action="{{ route('superadmin.schools.impersonate', $school->id) }}" method="POST" class="inline">
                                    @csrf
                                    <button type="submit" class="px-3 py-1.5 text-xs font-bold bg-amber-500 hover:bg-amber-600 text-slate-950 rounded-xl transition-all duration-150 flex items-center gap-1.5 shadow-sm shadow-amber-500/20" title="Direct Login ke Tenant">
                                        <i class="fa-solid fa-right-to-bracket"></i>
                                        <span>Direct Login</span>
                                    </button>
                                </form>
                                <form action="{{ route('superadmin.schools.toggle-active', $school->id) }}" method="POST" class="inline">
                                    @csrf
                                    <button type="submit" class="px-3 py-1.5 text-xs font-bold rounded-xl border transition-all duration-150 {{ $school->is_perpus_active ? 'border-rose-200 dark:border-rose-800 text-rose-600 hover:bg-rose-50 dark:hover:bg-rose-950/20' : 'border-emerald-200 dark:border-emerald-800 text-emerald-600 hover:bg-emerald-50 dark:hover:bg-emerald-950/20' }}">
                                        {{ $school->is_perpus_active ? 'Nonaktifkan' : 'Aktifkan' }}
                                    </button>
                                </form>
                                <a href="{{ route('superadmin.admins.index', ['school_id' => $school->id]) }}" class="px-3 py-1.5 text-xs font-bold bg-indigo-50 dark:bg-indigo-950/40 text-indigo-600 dark:text-indigo-300 rounded-xl hover:bg-indigo-100 dark:hover:bg-indigo-900/40 transition-all duration-150 flex items-center gap-1">
                                    <i class="fa-solid fa-user-shield"></i>
                                    <span>Admins</span>
                                </a>
                            </div>
                ...
                @empty
                    <tr>
                        <td colspan="9" class="p-12 text-center text-slate-400 font-medium">Belum ada sekolah terdaftar. Silakan sinkronisasikan data absensi.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
