...
<div class="mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
    <div>
        <h2 class="text-2xl font-bold text-slate-800 dark:text-slate-100">Rekap Peminjaman Buku</h2>
        <p class="text-sm text-slate-400">Laporan dan rekapitulasi data sirkulasi peminjaman buku {{ $school->name ?? '' }}.</p>
    </div>
    <div>
        <a href="{{ route('perpus.reports.loans.print', request()->query()) }}" target="_blank" class="px-5 py-3 bg-indigo-600 hover:bg-indigo-700 active:scale-95 text-white font-semibold rounded-2xl shadow-lg shadow-indigo-600/20 transition-all duration-150 flex items-center gap-2 text-sm">
